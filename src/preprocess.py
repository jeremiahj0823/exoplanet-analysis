...
# Dropping all NAs for clean data.
rawDF = rawDF.dropna() 

# Decided to drop host's values.
rawDF = rawDF.drop("st_teff", axis=1) 
rawDF = rawDF.drop("st_rad", axis=1)
rawDF = rawDF.drop("st_mass", axis=1)
rawDF = rawDF.drop("st_lum", axis=1)

rawDF = rawDF[rawDF['pl_orbsmax'] <= 1.5] # Planet to host distance set to less than 1.5 AU, unused in scoring
...
rawDF = rawDF[rawDF['pl_orbeccen'] >= 0] # Planet orbital eccentricity set to greater than 0, unused in scoring
